...
export class ModerationService {
  ...
  // MAIN CHECK IMAGE
  // =========================
  async checkImage(filePath: string): Promise<{
    is_safe: boolean;
    reason?: string;
    unsafe_score?: any;
    labels?: string[];
    ...
    try {
      ...
      // ==================================================
      // CLASS 1 – SAFE SEARCH (NUDE / SEX / VIOLENCE)
      // ==================================================
      if (safe) {
        const adult = this.toLikelihood(safe.adult);
        const racy = this.toLikelihood(safe.racy);
      ...
      // ==================================================
      // CLASS 2 – LABEL KEYWORDS (WEAPON / BLOOD / VIOLENCE)
      // ==================================================
      const BLACKLIST_LABELS = [
        'blood',
        'bleeding',
      ...
      for (const label of labels) {
        ...
        if (score > 0.7 && BLACKLIST_LABELS.some((bad) => name.includes(bad))) {
          return {
            is_safe: false,
            reason: `Phát hiện vật thể/nội dung cấm: ${
              label.description
      ...
      // ==================================================
      // CLASS 3 – OCR / TEXT DETECTION (XXX, 18+, PORN…)
      // ==================================================
      const fullText = texts.length ? texts[0].description.toLowerCase() : '';
      ...
      // ⚠️ Vision API lỗi (billing, timeout, quota…)
      console.error('Vision API error:', err.message);

      // DEV MODE: không block user
      return { is_safe: true };

      // PROD STRICT MODE (nếu muốn):
      // return { is_safe: false, reason: 'IMAGE_MODERATION_ERROR' };
    }
  }
}
